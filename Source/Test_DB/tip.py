...
class Ui_TipWindow(object):
    ...
    def setupUi(self, lastWindow):
        ...
        self.page_record = DataUtil.get_page_record(user_id=1, restaurant_id=1, page_id=1)
        self.total_score = self.page_record.total_score

        self.mainwindow = lastWindow
        lastWindow.setStyleSheet(open('style.qss').read())
        lastWindow.setObjectName("tipWindow")
        lastWindow.resize(800, 600)

        # set background image
        window_pale = QtGui.QPalette()
        window_pale.setBrush(lastWindow.backgroundRole(), QtGui.QBrush(QtGui.QPixmap("bg.jpg")))
        lastWindow.setPalette(window_pale)
    ...
    def btn_clk(self):
        ...
        if DataUtil.save_bill(user_id=1, restaurant_id=1, page_id=1, bill_amount=int(text), tip=float('%.2f' % Tips), waiter=text2):
            QMessageBox.information(self.mainwindow,
                                    "Finish",
                                    'Thank you for grading ' + text2,
                                    QMessageBox.Ok)
        else:
